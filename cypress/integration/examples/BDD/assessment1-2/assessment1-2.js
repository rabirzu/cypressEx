...
When("I add products to cart", (product) => {
  cy.selectProduct(product);
  elementArray.push(product);
  cy.log(elementArray);
});

When("I add more products to cart", function (dataTable) {
  cy.log(dataTable);
...
When("I check if the products selected are in Cart", function (dataTable) {
  cy.log(dataTable);

  cy.get(".inventory_item_name").each(($el, index) => {
    $el.text().includes(dataTable.rawTable[index]);
  });
});
